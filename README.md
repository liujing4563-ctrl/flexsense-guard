...
截至 2026-07-22，仓库的关键事实是：

- 项目范围规范为 `APPROVED`；数学语义和跨语言接口规范为 `REVIEWED`，仍待模块
  同学独立批准，不是 `FROZEN`；
- Python/JSON Schema v2 公共契约已实现并通过契约测试；MATLAB 映射仍为
  `INVALID`，C 和 App 映射仍为 `MISSING`，所以跨语言实现只是 `PARTIAL`；
- 历史 P1 脚本曾运行出 `FAIL`，但证据为 `INVALID / INSUFFICIENT`；当前 P1-V 和
  P1-A 均为 `NOT_VERIFIED`，不能据此宣布 EKF 路线成功或失败；
- P2 和 P3 仍被 P1-V 阻断，不启动分类训练、完整 App、C/SIL 或 Test Agent 主体开发；
- Draft PR #16 已锁定修改范围并发出非作者审查请求，当前等待 Simulink 与
  软件/Linux 方向的可实现性审查，不进入合并流程。

当前协作顺序是：完成 PR #16 人工审查，审查通过后合并；再将 PR #15 同步到新的
`main`，最后由 Simulink 同学修复 MATLAB Plant、Observer 和 P1 输入链并重新复验。
...
- B 路线：P1-V 通过，但 P1-A、P2 或 P3 至少一项未通过；只保留对应子门已经验证
  的能力，不作完整可信安全声明。
- `P1_REFRAME_REVIEW`：P1-V 通过而 P1-A 未建立优势时，只能选择
  `CONTINUE_TO_ROUTE_B`、一次 `ONE_BOUNDED_REPAIR` 或 `ENTER_ROUTE_C`；不得无限
  调整指标或重复使用已解封评价集调参。
- C 路线：可由有效且可复验的 P1-V 失败触发，也可在 P1-V 已通过后通过独立
  Decision ID 主动选择；后一种情况是范围决定，不能把 P1-V 改写为失败。
