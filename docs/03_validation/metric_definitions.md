...
| `external_torque_rmse_nm` | `sqrt(mean((estimated_external_torque_nm-true_external_torque_nm)^2))` | N·m | 冻结外扰评价窗口 | 无外扰真值、非有限或样本不足时不计算并标原因 | 中位数、Q1、Q3、失败数 | P1/P2 | Validation | Classification 评价、报告 |
...
| `false_alarm_count` | 无真实接触事件被判为接触的合并事件数量 | count | 完整案例及冻结事件合并窗口 | 缺标签或合并规则时无效 | 总数、中位数、Q1、Q3、失败数 | P2/P3 | Validation | Acceptance、报告 |
| `missed_detection_count` | 真实接触未在冻结检测窗口内形成有效检出的事件数量 | count | 每个真实接触事件的检测窗口 | 缺标签或窗口时无效 | 总数和逐案例分布 | P2/P3 | Validation | Acceptance、报告 |
...
事件必须先按冻结合并窗口形成事件级记录，不能按采样点重复累计误报或漏报。
