...
- 旧字段 `motor_torque_nm` 自本版本起视为待迁移名称，在 Python、MATLAB 和
  Schema 完成同步前，输入契约状态保持 `NOT FROZEN`。
...
当前文档已经冻结目标字段和语义，但 Python 输入类型、MATLAB 输入结构和输入
Schema 尚未完成迁移，因此公共输出契约保持候选冻结，公共输入契约为
`NOT FROZEN`。
