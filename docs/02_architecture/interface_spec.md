...
本规范冻结的是公共字段语义，不表示所有生产者和消费者已经完成迁移。当前状态为：

- 文档、JSON Schema 和 Python 候选契约：已对齐；
- MATLAB Plant、Observer 和 P1 runner：尚未迁移；
- 跨语言公共输入契约：`NOT FROZEN`；
- 端到端数据链：`NOT_VERIFIED`。

MATLAB 迁移的任务、禁止范围和验收标准见
[`p1_model_input_handoff.md`](../04_collaboration/p1_model_input_handoff.md)。迁移完成前，
代码中的旧内部变量只能视为待修复实现，不能反向覆盖本规范。
